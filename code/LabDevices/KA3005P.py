...
class KA3005P:
  def __init__(self, serial_port):
    try:
        self.serial = serial.Serial(serial_port, baudrate=9600, timeout=0.05)
    except serial.SerialException:
        print(f"Port {serial_port} is not available.")
        exit(1)

    # check if device is available
    self.serial.write('*IDN?'.encode())
    time.sleep(0.1)
    response = self.serial.readline().decode().strip()
    # print(f"Response to *IDN?: {response}")
    if "KA3005" not in response:
      print("Device is not available. Exiting.")
      exit(1)
