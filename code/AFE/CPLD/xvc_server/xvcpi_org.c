...
static int tms_gpio = 26;
static int tck_gpio = 27;
static int tdo_gpio = 0;
static int tdi_gpio = 1;

static int verbose = 0;

/* Transition delay coefficients */
#define JTAG_DELAY (500)
static unsigned int jtag_delay = JTAG_DELAY;

static int bcm2835gpio_read(void)
{
   return (0x1 & (GPIO_LEV >> tdo_gpio));
}
...
static bool bcm2835gpio_init(void)
{
   unsigned int bcm2835_peri_base = 0xFE000000;//bcm_host_get_peripheral_address();
   unsigned int bcm2835_peri_size = 0x1000;//bcm_host_get_peripheral_size();
   ...
   if (verbose) {
      printf("address=%08x size=%08x\n", bcm2835_peri_base, bcm2835_peri_size);
   }
